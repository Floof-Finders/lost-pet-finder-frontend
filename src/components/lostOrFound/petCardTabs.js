				...
				<div className='cardTabs'>
					<Row xs={5} md={3} className='g-4 petCards'>
						{petArray.pets &&
							petArray.pets
								.filter(
									(lost) => lost.lostOrFound === true && lost.isLost === true
								)
								.map((pet, id) => (
									<Col key={id}>
										<Card style={{ width: '18rem' }}>
											...
											<Card.Body>
												<Card.Title>Pet Name: {pet.petName}</Card.Title>
												<Card.Title>
				...
				<div className='cardTabs'>
					<Row xs={5} md={3} className='g-4 petCards'>
						{petArray.pets &&
							petArray.pets
								.filter(
									(lost) => lost.lostOrFound === false && lost.isLost === true
								)
								.map((pet, id) => (
									<Col key={id}>
										<Card style={{ width: '18rem' }}>
											...
											<Card.Body>
												<Card.Title>
													{pet.petName} -- Reward: ${' '}
													{pet.reward ? pet.reward : 'No Reward'}
												</Card.Title>
												<Button
													variant='primary'
													onClick={() => {
